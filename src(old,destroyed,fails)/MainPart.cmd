...
:: ===============================================================================================
::      Check: Already downloaded / pages missing? / History + name of the comic  /  Prüfen: Bereits heruntergeladen / Seiten fehlen? / History + Name des Comics #
:: ===============================================================================================
:: Check if comic has already been downloaded (if yes, load variables from DB)
:: Prüfen, ob Comic bereits heruntergeladen wurde (wenn ja, lade Variablen aus DB)
:: Set comic name to save downloads
:: Comicnamen zum Speichern der Downloads festlegen
:: ===============================================================================================
::      Start of the data query from the user and from the website 2  /  Start der Datenabfrage vom User und von der Webseite 2
:: ===============================================================================================
:: Set or confirm the number of pages
:: Seitenanzahl festlegen oder bestätigen
    IF exist "%savePathDatabaseFolderComicID%\finished.txt" (
      IF EXIST "src\ask_pages_renew_load.cmd" (
        CALL "src\ask_pages_renew_load.cmd"
      ) ELSE (
          CLS & COLOR 0C
          ECHO Fatal ERROR by loading "src\ask_pages_renew_load.cmd", file do not exist & echo Downloads the missing file or the latest version of the software from: & echo https://github.com/RaptorXilef/ImHentai.xxx_Image_Downloader.
          PAUSE & EXIT
      )
    ) ELSE (
:: Unterverzeichnis zum speichern der Downloads festlegen
      CALL "src\ask_downloadFolder.cmd"
:: Seitenanzahl festlegen oder bestätigen
      CALL "src\ask_pages.cmd"
    )
...
:: Comicname festlegen oder bestätigen
    IF not exist "%savePathDatabaseFolderComicID%\finished.txt" CALL "src\ask_comicName.cmd"
:: Datenprüfung
