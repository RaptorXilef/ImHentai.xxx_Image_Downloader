    ...
    findstr /L "<h1>" "%htmlVAR%" >"%tempVAR%"
        set /p temp=<"%tempVAR%"
            ...
            ::Del Russian letter (kompatibility)
                ::CALL "resources\anti_kyrillisch.cmd"
                ::echo %temp% >"%tempVAR%"
            set "comic-name_input=%temp:~1%"
    ::DEBUG echo %comic-name_input% && pause
...
:: Artist
    ...
    FOR /F "usebackq tokens=4 delims=/" %%A IN ("%tempVAR%") DO set VARIABLE2=%%A
